...
const typeMeta: Record<EntityType, { label: string; color: string; icon: string }> = {
  ...
  podcast: { label: 'Video podcast', color: '#8b3d49', icon: '◍' },
...
const domainMeta: Record<DomainKey, { label: string; shortLabel: string; color: string; types: EntityType[] }> = {
  ...
  screen: { label: 'Film & Television', shortLabel: 'Screen', color: '#8b3d49', types: ['film', 'series', 'podcast'] },
...
const domainRegionBounds: Record<GraphLayer, Record<DomainKey, RegionBounds>> = {
  overview: {
    ...
    screen: { x: 550, y: 470, width: 480, height: 175 },
  ...
  groups: {
    ...
    screen: { x: 550, y: 470, width: 480, height: 160 },
  ...
  detail: {
    ...
    screen: { x: 560, y: 480, width: 450, height: 590 },
...
const isScreenRecord = (entity: Entity) => domainMeta.screen.types.includes(entity.type);
...
const entityPositions: Record<string, { x: number; y: number }> = {
  ...
  'series-in-the-soop-bts': { x: 590, y: 885 }, 'series-in-the-soop-bts-2': { x: 790, y: 885 },
  'podcast-fashion-neurosis-rm': { x: 690, y: 960 },
...
const groupDefinitions: { key: string; domain: DomainKey; label: string; types: EntityType[]; x: number; y: number }[] = [
  ...
  { key: 'screen-stories', domain: 'screen', label: 'Films & series', types: ['film', 'series'], x: 670, y: 558 },
  { key: 'screen-conversations', domain: 'screen', label: 'Video conversations', types: ['podcast'], x: 865, y: 558 },
...
function Layout({ children, theme, onThemeChange }: { children: React.ReactNode; theme: Theme; onThemeChange: () => void }) {
  ...
  return <div className={`app-shell theme-${theme}`}>
    <header className="site-header">
      ...
      <div className="header-tools"><button className="theme-switch" onClick={onThemeChange} data-umami-event="theme-toggle" data-umami-event-theme={theme === 'science' ? 'funside' : 'science'} aria-label={`Switch to ${theme === 'science' ? 'Funside' : 'Science'} colour palette`} aria-pressed={theme === 'funside'}><span className="theme-dot" />{theme === 'science' ? 'Science' : 'Funside'}<span className="theme-switch-arrow">↗</span></button><span className="header-note">Public research atlas</span></div>
    </header>
    <PrincipleBanner />
    <main>{children}</main>
    <footer className="site-footer"><span>The Namjoon (RM) Index</span><span>Independent · Unofficial · Source-based</span><span><Link to="/about">Methodology</Link> · <Link to="/privacy">Privacy</Link></span></footer>
  </div>;
}
...
function EditorialCover({ entity, compact = false }: { entity: Entity; compact?: boolean }) {
  const coverIndex = (entities.filter(isScreenRecord).indexOf(entity) % 6) + 1;
...
function ResearchFindingCard({ finding, compact = false }: { finding: ResearchFinding; compact?: boolean }) {
  return <article className={`research-finding-card status-${finding.status} ${compact ? 'compact' : ''}`}>
    ...
    <div className="research-sources"><span>Found through</span>{finding.sources.map((source) => <a href={source.url} target="_blank" rel="noreferrer" data-umami-event="source-open" data-umami-event-source={source.label} key={`${finding.id}-${source.url}`}><strong>{source.label} ↗</strong><small>{source.role}{source.locator ? ` · ${source.locator}` : ''}</small></a>)}</div>
  </article>;
}
...
function AttributionTrail({ relation, compact = false }: { relation: Relation; compact?: boolean }) {
  ...
  return <div className={`attribution-trail ${compact ? 'compact' : ''}`}>
    {discoveries.length > 0 && <div className="attribution-step community-step"><span className="attribution-label">Discovered by</span><div>{discoveries.map(({ credit, source }) => <div className="credit-entry" key={`${source.id}-${credit.role}`}><a href={source.url} target="_blank" rel="noreferrer" data-umami-event="source-open" data-umami-event-source={source.id}>{discoveryRoleMeta[credit.role]} {source.publisher} ↗</a>{credit.locator && <small>{credit.locator}</small>}{!compact && <p>{credit.note}</p>}</div>)}</div></div>}
    <div className="attribution-step verified-step"><span className="attribution-label">Verified with</span><div>{verifiedSources.map((source) => <a className="source-inline" href={source.url} target="_blank" rel="noreferrer" data-umami-event="source-open" data-umami-event-source={source.id} key={source.id}>{source.publisher} — {source.title} ↗</a>)}</div></div>
  </div>;
}
...
function GraphView({ compact = false, domain }: { compact?: boolean; domain?: DomainKey }) {
  ...
  return <div className={`graph-wrap ${compact ? 'compact' : ''}`}>
    <div className="graph-toolbar"><div className="toolbar-label"><span>Semantic map</span><b>{domain ? domainMeta[domain].label : layerLabel[graph.layer]}</b>{domain && <button onClick={() => navigate('/map')}>All areas ×</button>}</div><div className="graph-legend"><button className={evidence.includes('confirmed') ? 'selected' : ''} data-umami-event="map-evidence-filter" data-umami-event-level="confirmed" onClick={() => setEvidence((old) => old.includes('confirmed') ? old.filter((x) => x !== 'confirmed') : [...old, 'confirmed'])}><i className="legend-line confirmed" /><span>Confirmed <b>{evidenceCounts.confirmed}</b></span></button><button className={evidence.includes('mentioned') ? 'selected' : ''} data-umami-event="map-evidence-filter" data-umami-event-level="mentioned" onClick={() => setEvidence((old) => old.includes('mentioned') ? old.filter((x) => x !== 'mentioned') : [...old, 'mentioned'])}><i className="legend-line mentioned" /><span>Mentioned <b>{evidenceCounts.mentioned}</b></span></button><button title={evidenceMeta.editorial.hint} className={evidence.includes('editorial') ? 'selected' : ''} data-umami-event="map-evidence-filter" data-umami-event-level="editorial" onClick={() => setEvidence((old) => old.includes('editorial') ? old.filter((x) => x !== 'editorial') : [...old, 'editorial'])}><i className="legend-line editorial" /><span>Index interpretation <b>{evidenceCounts.editorial}</b></span></button></div></div>
...
function ScreenPage() {
  usePageMeta('Films & Television | The Namjoon (RM) Index', 'Browse films, television, video podcasts and filmed conversations connected to RM, with each watch, appearance and recommendation kept at the level its source supports.');
  const screenEntities = entities.filter(isScreenRecord);
  ...
  return <div className="screen-page">
    <section className="screen-hero"><div className="screen-hero-copy"><p className="eyebrow">Films & television</p><h1>A screen life,<br />carefully traced.</h1><p>Films RM returned to, series and programmes in which he appears, video podcasts, and a documentary made around his own work—with labels that keep “watched,” “featured in” and “recommended” distinct.</p><div className="screen-stats"><div><strong>{screenEntities.length}</strong><span>screen records</span></div><div><strong>{confirmed}</strong><span>confirmed records</span></div><div><strong>{mentioned}</strong><span>traced mentions</span></div></div></div><div className="screen-frame" aria-hidden="true"><span>SCREEN / 001</span><div><i /><i /><i /></div><strong>24<br />FPS</strong><small>Every frame needs a locator.</small></div></section>
    ...
    <section className="screen-catalog"><div className="section-heading"><div><p className="eyebrow">Filmography / watchlist / conversations</p><h2>{screenEntities.length} records, {screenEntities.length} distinct trails</h2></div><Link className="text-link" to="/map?domain=screen">See them on the map →</Link></div><p className="screen-intro">The card label describes what the source can support. In the SOOP is recorded as an appearance; Fashion Neurosis is a confirmed video-podcast appearance; a later compilation without a primary locator remains only a mention even when it calls something a favourite or recommendation.</p><div className="screen-grid">{screenEntities.filter((entity) => entity.id !== featured?.id).map((entity, index) => { const relation = getRelationsFor(entity.id).find((item) => item.from === 'rm' || item.to === 'rm'); if (!relation) return null; return <Link className="screen-card" to={`/entity/${entity.slug}`} key={entity.id}><div className={`screen-card-cover cover-${(index % 6) + 1}`}><span>{typeMeta[entity.type].label}</span><strong>{entity.title}</strong><small>{entity.creator} · {entity.year}</small></div><div className="screen-card-copy"><div><EvidenceBadge level={relation.evidenceLevel} /><span>{relationLabel(relation.type)}</span></div><h3>{entity.title}</h3><p>{relation.note}</p><span className="screen-open">Open record ↗</span></div></Link>; })}</div></section>
    <section className="screen-protocol"><span>Screen protocol / 01</span><h2>Watching is not endorsement.<br />Appearing is not recommending.</h2><p>The index records the narrowest defensible relationship. When a primary broadcast, post or interview cannot yet be opened at a precise point, the work stays visible with a weaker label and the missing locator stated in plain language.</p><Link className="text-link" to="/about">Read the full methodology →</Link></section>
  </div>;
}
...
function Explore() {
  ...
  return <div className="content-page"><PageIntro eyebrow="Explore" title="The index, in pieces." description="Browse the entities and documented relationships that make up this growing cultural map." /><div className="explore-controls"><label className="search-field"><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search title, creator or alternate name" aria-label="Search the index" /></label><div className="filter-row">{allTypes.map((item) => <button key={item.key} data-umami-event="explore-filter" data-umami-event-filter={item.key} onClick={() => setFilter(item.key)} className={filter === item.key ? 'filter active' : 'filter'}>{item.label}</button>)}</div><div className="evidence-filter"><span>Evidence</span>{(['confirmed', 'mentioned', 'editorial'] as EvidenceLevel[]).map((level) => <label key={level}><input type="checkbox" data-umami-event="explore-evidence-filter" data-umami-event-level={level} checked={evidence.includes(level)} onChange={() => setEvidence((old) => old.includes(level) ? old.filter((item) => item !== level) : [...old, level])} /><EvidenceBadge level={level} /></label>)}</div></div><div className="results-head"><span>{visible.length} results</span></div><div className="entity-grid">{visible.map((entity) => <EntityCard key={entity.id} entity={entity} />)}</div>{visible.length === 0 && <div className="empty-state"><span>∅</span><h3>No documented entities found</h3><p>Try a different search or evidence filter.</p></div>}</div>;
}
...
function EntityCard({ entity }: { entity: Entity }) {
  ...
  return <Link className="entity-card" to={`/entity/${entity.slug}`}>
    ...
    <div className={`card-visual ${entity.image ? 'has-image' : ''}`} style={{ '--card-color': meta.color } as React.CSSProperties}>
      {entity.image ? <img src={entity.image.url} alt="" loading="lazy" /> : isScreenRecord(entity) ? <EditorialCover entity={entity} compact /> : <span>{entity.title.slice(0, 1)}</span>}
...
function TimelineEventCard({ event }: { event: TimelineEvent }) {
  ...
  return <article className="timeline-event" style={{ '--event-color': timelineCategoryMeta[event.category].color } as React.CSSProperties}>
    ...
    <div className="timeline-sources">{eventSources.map((source, index) => <a className="source-inline" href={source.url} target="_blank" rel="noreferrer" data-umami-event="source-open" data-umami-event-source={source.id} key={source.id}>{index === 0 ? 'Source' : 'Also'}: {source.publisher} ↗</a>)}</div>
  </article>;
}
...
function Timeline() {
  ...
  return <div className="content-page timeline-page"><PageIntro eyebrow="Timeline" title="Moments, not timestamps." description="A chronology of documented cultural episodes. Related connections are grouped together, and uncertain dates remain visibly uncertain." />
    ...
    <div className="timeline-controls"><span>Show</span><button className={category === 'all' ? 'active' : ''} data-umami-event="timeline-filter" data-umami-event-category="all" onClick={() => setCategory('all')}>All episodes</button>{categories.map((key) => <button key={key} className={category === key ? 'active' : ''} data-umami-event="timeline-filter" data-umami-event-category={key} onClick={() => setCategory(key)} style={{ '--filter-color': timelineCategoryMeta[key].color } as React.CSSProperties}>{timelineCategoryMeta[key].label}</button>)}</div>
...
function SourcesPage() {
  ...
  return <div className="content-page sources-page">
    ...
    <section className="community-archives-section">
      ...
      <div className="community-archive-grid">{communityArchives.map((archive) => <a className="community-archive-card" href={archive.url} target="_blank" rel="noreferrer" data-umami-event="source-open" data-umami-event-source={archive.id} key={archive.id}><div className="community-card-top"><span>{archive.archiveType.replaceAll('_', ' ')}</span><i>↗</i></div><h3>{archive.title}</h3><div className="community-domains">{archive.domains.map((domain) => <span key={domain}>{domain.replaceAll('_', ' ')}</span>)}</div><p>{archive.sourceLinkage}</p></a>)}</div>
    </section>
...
function Privacy() {
  usePageMeta('Privacy | The Namjoon (RM) Index', 'How The Namjoon Index uses privacy-friendly, cookie-free analytics to understand aggregate site use and performance.');
  return <div className="content-page about-page privacy-page">
    <PageIntro eyebrow="Privacy" title="Useful statistics, without personal profiles." description="The index uses aggregate, cookie-free analytics to understand which parts of the archive are useful and where the site can be improved." />
    <div className="about-grid">
      <div>
        <h2>Analytics we use</h2>
        <p>We use Cloudflare Web Analytics for traffic and performance measurements, and Umami Cloud for aggregate page journeys and selected interactions. These services are not used to identify visitors, build advertising profiles or track people across websites.</p>
        <p>Umami may process the page path, referrer, approximate country, browser, operating system, device type, language and screen size. We also record a small set of anonymous events: filter changes, theme changes and clicks that open an external research source.</p>
        <h2 className="small-heading">What we do not collect</h2>
        <p>The analytics configuration does not use cookies or local storage. We do not send names, email addresses, account identifiers or terms typed into the site search. The Umami tracker is configured to respect the browser’s Do Not Track preference.</p>
      </div>
      <div>
        <h2>Retention and providers</h2>
        <p>Umami Cloud’s current Hobby plan retains analytics data for six months. Cloudflare and Umami process analytics according to their own security, privacy and retention documentation.</p>
        <p><a className="text-link" href="https://www.cloudflare.com/web-analytics/" target="_blank" rel="noreferrer">Cloudflare Web Analytics privacy information ↗</a></p>
        <p><a className="text-link" href="https://umami.is/privacy" target="_blank" rel="noreferrer">Umami privacy information ↗</a></p>
        <h2 className="small-heading">Cookies and future changes</h2>
        <p>No analytics cookie consent banner is displayed because the current analytics setup does not store analytics cookies on a visitor’s device. If the project later adds accounts, advertising, forms or analytics that require consent, this notice and the site controls will be updated before those tools are enabled.</p>
      </div>
    </div>
    <p className="privacy-updated"><span>Last updated</span> 13 August 2026</p>
  </div>;
}

type AuthorProfile = { name: string; role: string; story: string; links: { label: string; url: string }[] };

const indexAuthors: AuthorProfile[] = [
  { name: 'Di Vien', role: 'AI Engineer', story: 'Personal note — coming soon.', links: [{ label: 'LinkedIn', url: 'https://www.linkedin.com/in/dvien/' }, { label: 'advien.tech', url: 'https://advien.tech/' }] },
  { name: 'Nata Kasatkina', role: 'Analytics and AI', story: 'Personal note — coming soon.', links: [{ label: 'LinkedIn', url: 'https://www.linkedin.com/in/kasatkinan/' }, { label: 'excelebro.com', url: 'https://excelebro.com/' }] },
];
