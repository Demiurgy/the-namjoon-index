...
const domainRegionBounds: Record<GraphLayer, Record<DomainKey, RegionBounds>> = {
  ...
  detail: {
    literature: { x: 20, y: 20, width: 500, height: 300 },
    'visual-art': { x: 820, y: 20, width: 680, height: 380 },
    music: { x: 20, y: 470, width: 500, height: 140 },
    screen: { x: 570, y: 490, width: 410, height: 120 },
    institutions: { x: 1040, y: 460, width: 480, height: 300 },
  },
};
...
const entityPositions: Record<string, { x: number; y: number }> = {
  rm: { x: 650, y: 415 },
  'book-depth-of-the-landscape': { x: 60, y: 95 }, 'book-honeybees-distant-thunder': { x: 290, y: 95 },
  'book-that-summers-end': { x: 60, y: 170 }, 'book-all-about-saul-leiter': { x: 290, y: 170 },
  'book-midnight-library': { x: 60, y: 245 }, 'book-doing-good-better': { x: 290, y: 245 },
  'yun-hyong-keun': { x: 850, y: 90 }, 'kim-whanki': { x: 1065, y: 90 }, 'lee-ungno': { x: 1280, y: 90 },
  'kwon-dae-sup': { x: 850, y: 160 }, 'joung-young-ju': { x: 1065, y: 160 }, 'kang-yobae': { x: 1280, y: 160 },
  'untitled-1973-yun': { x: 850, y: 250 }, 'the-eternal-song': { x: 1065, y: 250 },
  'bamboo-lee-ungno': { x: 850, y: 325 }, 'moon-jar-kwon': { x: 1065, y: 325 }, 'artwork-kang-yobae-munnamu': { x: 1280, y: 250 },
  indigo: { x: 80, y: 530 }, mono: { x: 300, y: 530 },
  'rm-x-sfmoma': { x: 1080, y: 530 }, sfmoma: { x: 1280, y: 530 },
  'exhibition-kang-yobae-carnation': { x: 1080, y: 610 }, 'museum-daegu-art-museum': { x: 1280, y: 610 }, 'seoul-museum-of-art': { x: 1180, y: 690 },
};
...
function SourcesPage() {
  ...
  return <div className="content-page">
    ...
    <div className="source-summary"><strong>{sources.length}</strong><span>verification records</span><strong>{researchFindings.length}</strong><span>labelled open findings</span><strong>{communityArchives.length}</strong><span>community archives acknowledged</span><Link className="text-link" to="/about">Read source policy →</Link></div>
